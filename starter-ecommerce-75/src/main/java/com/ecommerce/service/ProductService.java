...
public class ProductService {
 private final ProductRepository products; private final UserService users; public ProductService(ProductRepository p,UserService u){products=p;users=u;}
 @Transactional(readOnly=true) public Page<ProductResponse> all(int page,int size){Pageable pageable=PageRequest.of(Math.max(0,page),Math.min(Math.max(size,1),100),Sort.by("id").descending());return products.findAll(pageable).map(this::response);}
 ...
 @Transactional public void delete(Long id,String name){products.delete(owned(id,name));}
